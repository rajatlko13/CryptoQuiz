...
contract QuizFactory {
    ...
    address[] public deployedQuizzes;
    
    function createQuiz(address addressEIP20) public returns (address) {
        address owner = msg.sender;
        Quiz newQuiz = new Quiz(addressEIP20, owner);
        deployedQuizzes.push(address(newQuiz));
        return address(newQuiz);
    }
    
    function getDeployedQuizzes() public view returns (address[] memory) {
        return deployedQuizzes;
    }
}
